...
|---|---|---|---|
| 2026-09-12 | #1 레버리지 | **50x~100x 레짐별 동적** (사용자 확정) · ⚠️ **재검토 대기(2026-09-22)**: 트라이얼 #1 청산 포렌식(#26) — 50~100x에서 SL↔청산 간격 ~0.2%는 1m 봉 하나가 흔히 관통, 26건 전부 28x 이하면 생존. 트라이얼 #2 사전등록 전 사용자 재정 | 이는 *허용 범위*이고 실효 레버리지는 SL 거리에서 도출(sizing v2)한 뒤 범위로 클램프한다. 100x·tier1 MMR 0.4%면 청산가는 진입가에서 LONG 0.6%·SHORT 0.6%다 — SL은 반드시 청산 거리 안쪽 + 버퍼(q_c=0.9995)에 놓여야 하고, 놓을 수 없으면 레버리지를 낮추거나 진입하지 않는다. `references/strategy-modules.md §1` |
