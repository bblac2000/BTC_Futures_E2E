...
from paper.sender import LiveChecklist, LiveSender, OrderOutcomeUnknown, PaperSender, adverse_fill_estimate
...
class SpySender:
    ...
    def quote_fill_price(self, side, ref_mark):
        return self.inner.quote_fill_price(side, ref_mark)             # PAPER·LIVE 같은 #7 추정(사용자 2026-09-15)
...
class NoQuoteSender(PaperSender):
    """실제 체결이 예상 체결가보다 나쁜 송신기 — 사이징은 mark로, 체결은 슬리피지만큼 불리하게(LIVE 슬리피지 초과 흉내)."""
    def quote_fill_price(self, side, ref_mark):
        return ref_mark
...
class ExactQuoteClient:
    """쓰기 클라이언트 흉내 — MARKET이 **정확히 LIVE 예상 체결가(#7)**에 체결되고, positionRisk는 체결 수량을 그대로 보인다."""
    def __init__(self, fill_price):
        self.fill_price, self.amt, self.calls = fill_price, D("0"), []

    def get(self, path, params=None, *, signed=False):
        self.calls.append(("GET", path))
        if path == "/fapi/v1/userTrades":
            return Response(200, [], {})                                 # 수수료는 런타임 taker 추정(표시)
        if path == "/fapi/v2/positionRisk":
            return Response(200, [{"symbol": "BTCUSDT", "positionSide": "BOTH", "positionAmt": str(self.amt),
                                   "entryPrice": str(self.fill_price), "liquidationPrice": "0"}], {})
    ...
    def post(self, path, params=None, *, signed=True):
        ...
        if path == "/fapi/v1/leverage":
            return Response(200, {"symbol": p["symbol"], "leverage": int(p["leverage"])}, {})
        q = D(p["quantity"])
        self.amt += q if p["side"] == "BUY" else -q
        return Response(200, {"orderId": len(self.calls), "status": "FILLED", "avgPrice": str(self.fill_price),
                              "executedQty": p["quantity"], "side": p["side"], "updateTime": DAY0 + 1000}, {})


@pytest.mark.parametrize("direction", [LONG, SHORT])
def test_live_fill_exactly_at_the_registry_7_estimate_passes_the_post_fill_gate(rules, direction):
    """사용자 결정(2026-09-15): LIVE 사이징은 불리한 체결을 가정한다(#7). 실제 체결이 그 추정과 **정확히 같으면**
    체결 후 #5 검사가 통과하고 청산하지 않는다 — PAPER와 같은 코드 경로."""
    mark = D("60000")
    side = Side.BUY if direction is LONG else Side.SELL
    quote = adverse_fill_estimate(side, mark, rules.symbol_rules.tick_size)
    client = ExactQuoteClient(quote)
    s = LiveSender(client, rules, mode=Mode.LIVE, checklist=LiveChecklist(**{f.name: True for f in fields(LiveChecklist)}))
    assert s.quote_fill_price(side, mark) == quote and quote != mark
    e = Engine(rules, s, mode=Mode.LIVE, wallet=W0, limits=SizingLimits())
    e.request_entry(intent(direction))
    ev = e.on_tick(tick(DAY0 + 1000, mark))
    (fill,) = of(ev, EntryFilled)
